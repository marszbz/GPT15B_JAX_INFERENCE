...
try:
    ...
    import numpy as np
    print("✅ JAX " + jax.__version__ + " HLO语法解释模式")
except ImportError as e:
    print("❌ JAX导入失败: " + str(e))
    sys.exit(1)

class FixedHLOSyntaxExplainer:
    """修复版HLO语法详细解释器"""
...
ENTRY main.5 {
  ...
  ROOT %dot = f32[8,32] dot(f32[8,16] %x, f32[16,32] %w),
...
ENTRY main {
  ...
  ROOT %fusion = f32[8,32] fusion(%x, %w, %b), 
    ...
    def explain_sharding_semantics(self):
        ...
        print("   分片结果:")
        print("   ┌───────────┬───────────┐")
        print("   │D0:(4,16)  │D1:(4,16)  │ ← 第0维分片")
        print("   ├───────────┼───────────┤")
        print("   │D2:(4,16)  │D3:(4,16)  │")
        print("   └───────────┴───────────┘")
        print("            第1维分片")
    ...
    def show_jax_to_hlo_conversion(self):
        ...
        with mesh:
            ...
            print("\n🔍 生成的HLO结构 (简化版):")
            print("   HloModule jit_simple_matmul")
            print("   ")
            print("   ENTRY main {")
            print("     %x = f32[8,16] parameter(0)")
            print("          /*sharding={devices=[2,2] layout=[0,-1]}*/")
            print("     %w = f32[16,32] parameter(1)")
            print("          /*sharding={devices=[2,2] layout=[-1,1]}*/")
            print("     ROOT %dot = f32[8,32] dot(%x, %w),")
            print("                          lhs_contracting_dims={1},")
            print("                          rhs_contracting_dims={0}")
            print("                          /*sharding={devices=[2,2] layout=[0,1]}*/")
            print("   }")
            
            print("\n✅ 转换成功!")
            print("   输入: x" + str(x.shape) + " @ w" + str(w.shape))
            print("   输出: result" + str(result.shape))
...
def main():
    """主函数"""
    explainer = FixedHLOSyntaxExplainer()
