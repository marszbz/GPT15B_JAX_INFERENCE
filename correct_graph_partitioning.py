...
class DetailedGraphPartitioner:
    ...
    def create_device_mesh(self):
        ...
        try:
            ...
            # 获取mesh形状信息
            mesh_shape_dict = self.mesh.shape
            axis_names = self.mesh.axis_names
            
            print(f"   网格形状: {mesh_shape_dict}")
            print(f"   轴名称: {axis_names}")
            
            # 打印设备分配详情
            print(f"   设备分布:")
            if len(axis_names) == 2 and 'model' in axis_names:
                # 2D mesh情况
                data_size = mesh_shape_dict[self.config.data_axis]
                model_size = mesh_shape_dict[self.config.model_axis]
    ...
    try:
        ...
        if mesh_success:
            ...
            results = {
                ...
                'mesh_info': {
                    'shape': dict(partitioner.mesh.shape),
            ...
            print(f"   网格配置: {dict(partitioner.mesh.shape)}")
